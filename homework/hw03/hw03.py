...
def balanced(m):
    ...
    if is_planet(m):
        return True
    return balanced(end(left(m))) and balanced(end(
        right(m))) and total_weight(end(left(m))) * length(
...
def replace_leaf(t, find_value, replace_value):
    ...
    new_branches = []
    for i in branches(t):
        new_branches.append(replace_leaf(i, find_value, replace_value))
...
def quadratic(x, a, b, c):
    ...
    extremum = -b / (2 * a)
    y1, y2, y_ex = map(f, [lower_bound(x), upper_bound(x), extremum])
    if extremum < upper_bound(x) and extremum > lower_bound(x):
        return interval(min(y1, y2, y_ex), max(y1, y2, y_ex))
    return interval(min(y1, y2), max(y2, y1))
