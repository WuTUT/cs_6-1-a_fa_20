...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    selected_city = make_city('selected', lat, lon)
    return get_name(city_a) if distance(selected_city, city_a) < distance(
        selected_city, city_b) else get_name(city_b)
...
def sprout_leaves(t, leaves):
    ...
    branch = []
    for i in branches(t):
        branch.append(sprout_leaves(i, leaves))
...
def riffle(deck):
    ...
    "*** YOUR CODE HERE ***"
    return [
        deck[i // 2] if i % 2 == 0 else deck[i // 2 + len(deck) // 2]
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) and is_leaf(t2):
        ...
        subtrees = []
        lent1 = len(branches(t1))
        lent2 = len(branches(t2))
        for i in range(min(lent1, lent2)):
            subtrees.append(add_trees(branches(t1)[i], branches(t2)[i]))
        if lent1 < lent2:
            for i in range(lent1, lent2):
                subtrees.append(branches(t2)[i])
        else:
            for i in range(lent2, lent1):
                subtrees.append(branches(t1)[i])
        return tree(label(t1) + label(t2), subtrees)
