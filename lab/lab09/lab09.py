...
def subseqs(s):
    ...
    if not s:
        ...
        ans = subseqs(s[1:])
        return insert_into_all(s[0],ans) + ans
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n<=2:
        return 1
    return sum(num_trees(n-m)*num_trees(m) for m in range(1,n))
...
def make_advanced_counter_maker():
    ...
    def counter():
        own_count = 0
        def f(messages):
            nonlocal own_count,global_count
            if messages == 'count':
