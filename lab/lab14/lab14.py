...
def num_splits(s, d):
    ...
    def helper(a,b,s):
        if len(s) == 0:
            return 1 if abs(b-a)<=d else 0
        return helper(a+s[0],b,s[1:]) + helper(a,b+s[0],s[1:])
...
class CheckingAccount(Account):
    ...
    def deposit_check(self,check):
        if self.holder == check.holder and not check.deposited :
...
def filterl(lst, pred):
    ...
    def filtered(xs,x):
        if pred(x):
            ...
            if xs is Link.empty:
                return x
            return Link(xs,Link(x))
        return xs

    return foldl(lst,filtered,Link.empty)

def filterr(lst, pred):
    """ Filters LST based on PRED
    >>> lst = Link(4, Link(3, Link(2, Link(1))))
    >>> filterl(lst, lambda x: x % 2 == 0)
    Link(4, Link(2))
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldl(lst,lambda x,l: Link(l,x),Link.empty)
