...
def swine_align(player_score, opponent_score):
    ...
    if player_score > 0 and opponent_score > 0 and gcd(player_score,
                                                       opponent_score) >= 10:
        return True
    return False
    # END PROBLEM 4a
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if who == 0:
            gains = score0 - last_score
            if gains > running_high:
                new_running_high = gains
                print(new_running_high, 'point(s)! The most yet for Player 0')
            return announce_highest(who, score0, new_running_high)
        else:
            gains = score1 - last_score
            if gains > running_high:
                new_running_high = gains
                print(new_running_high, 'point(s)! The most yet for Player 1')
...
def make_averaged(original_function, trials_count=1000):
    ...
    def averaged(*args):
        ...
        while n < trials_count:
            averaged_value += original_function(*args)
            n += 1
        return averaged_value / trials_count
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    best_score = 0
    while num_rolls <= 10:
        tmp_score = make_averaged(roll_dice, trials_count)(num_rolls, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if gain >= cutoff or extra_turn(score + gain, opponent_score):
        return 0
    return num_rolls  # Replace this statement
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    best_num_rolls = 6
    if abs(100 - score) < 15:
        return extra_turn_strategy(score, opponent_score, 5, 3)
    return extra_turn_strategy(score, opponent_score, 20, best_num_rolls)
    # END PROBLEM 12
